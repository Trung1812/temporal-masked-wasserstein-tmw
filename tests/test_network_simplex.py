...
def main():
    ...
    if args.config:
        ...
        args.dataset_dir = config.get('dataset_root')
        args.log_dir = config.get('log_root', 'logs')
        args.n_trials = config.get('n_trials', 100)
    ...
    D_test = compute_tmw_train_test(
        X_train, X_test, 25, 0.0357066262467603 #best_w, best_lam
    )
